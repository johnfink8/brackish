...
```sh
brackish artifact list <thread>                  # see all artifacts and their states
brackish artifact get <thread> <name>            # latest accepted, content to stdout
brackish artifact get <thread> <name> --proposed # latest in-flight
brackish artifact get <thread> <name> --version 3
```
...
`brackish wait <thread> --timeout 60` blocks for up to 60 seconds and returns the moment a new event arrives (or empty + exit 0 on timeout). Cursors are server-tracked per (identity, thread) so a bare `brackish wait <thread>` means "since I last looked."

## Anatomy

- **CLI + daemon = one Node binary.** `brackish serve` is just a subcommand.
- **Storage:** append-only events table; threads + artifact state are projections. SQLite via `better-sqlite3`.
